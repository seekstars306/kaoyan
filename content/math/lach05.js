...
secs: [
['why', '为什么学这一节',
`<p>特征值刻画矩阵"固有的伸缩方向"：$A\\xi=\\lambda\\xi$ 表示向量 $\\xi$ 被矩阵作用后<b>只缩放不转向</b>。它是相似对角化、二次型正定、甚至计组与网络分析（PageRank）的共同源头，每年必考。</p>`],
['think', '直观理解',
`<p><b>找"矩阵的方向锚"。</b>大多数向量被 $A$ 作用后会转向，但总有几个特殊方向原地不动只拉长/缩短——这就是特征方向，缩放倍数 $\\lambda$ 就是特征值。</p>
<blockquote>求法一句话：<b>解 $(\\lambda E-A)\\mathbf x=\\mathbf0$ 的非零解。</b>要非零解 → 系数行列式为零 → 特征方程 $\\det(\\lambda E-A)=0$。</blockquote>
<p><b>两条速记性质：</b>所有特征值之和 = 主对角线之和（迹）；之积 = 行列式。</p>`],
['def', '定义与公式',
`<p><b>① 定义：</b>$A\\xi=\\lambda\\xi\\ (\\xi\\neq\\mathbf0)$；$\\lambda$ 特征值，$\\xi$ 对应特征向量。</p>
<p><b>② 特征方程：</b>$\\det(\\lambda E-A)=0$（$n$ 阶矩阵有 $n$ 个特征值，计重数）。</p>
<p><b>③ 两个恒等式：</b></p>
$$ \\sum\\lambda_i=\\mathrm{tr}(A),\\qquad \\prod\\lambda_i=\\det A $$
<p><b>④ 派生矩阵的特征值：</b>$A$ 的 $\\lambda$ → $kA$ 为 $k\\lambda$；$A^m$ 为 $\\lambda^m$；$A^{-1}$ 为 $\\lambda^{-1}$；$A^*$ 为 $\\dfrac{\\det A}{\\lambda}$；$f(A)$ 为 $f(\\lambda)$。<b>特征向量不变。</b></p>
<p><b>⑤ 不同特征值的特征向量线性无关；一个特征值对应无穷多特征向量（基础系 = $n-r(\\lambda E-A)$ 个）。</b></p>`],
['ex', '例题精讲',
...
['def', '定义与公式',
`<p><b>① 相似定义与不变量：</b>$B=P^{-1}AP$；相似 ⇒ 特征多项式相同 ⇒ 特征值、$\\det$、迹、秩、各阶特征值重数都相同。</p>
<p><b>② 可对角化充要条件：</b>$A$ 有 $n$ 个线性无关的特征向量 $\\iff$ 每个 $k$ 重特征值都有 $k$ 个无关特征向量（$\\iff$ 对每个 $\\lambda_i$，$n-r(\\lambda_iE-A)=k_i$ 重数）。</p>
<p><b>③ 充分条件：</b>$A$ 有 $n$ 个互异特征值 ⇒ 必可对角化；实对称矩阵必可对角化。</p>
...
{id:'q2', q:'$A$ 与 $B$ 相似，则下列不一定成立的是？', opts:['特征值相同', '$\\det A=\\det B$', '$AB=BA$', '秩相同'], ans:2, exp:'相似矩阵一般不可交换；前三项（含迹）都是相似不变量。'},
