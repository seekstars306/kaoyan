...
<blockquote><b>截面法</b>反过来：先用水平面切 $\\Omega$，截面面积函数 $S(z)$ 已知时 $\\iiint f dV=\\int dz\\iint_{D_z}f dxdy$——"豆腐切片"。</blockquote>
...
$$ \\int_0^{2\\pi}d\\theta\\int_0^{1}r\\,dr\\int_{r^2}^{1}z\\,dz=\\pi\\int_0^1 r\\cdot\\frac{1-r^4}{2}dr=\\frac{\\pi}{2}\\left(\\frac12-\\frac16\\right)=\\frac{\\pi}{6} $$
</div>`],
['warn', '易错点',
`<p>• 柱面坐标 $dV=r\\,drd\\theta dz$、球面 $dV=r^2\\sin\\varphi\\,drd\\varphi d\\theta$——体积元" forget r / forget $r^2\\sin\\varphi$"是最大杀手。</p>
<p>• 球坐标 $\\varphi$ 从<b> z 轴正方向</b>量起（$[0,\\pi]$），别与极角 $\\theta$ 混。</p>`]
],
quiz: [
...
['think', '直观理解',
`<p><b>第一类：给曲线"称重"。</b>密度 $f$，微元质量 = 密度 × 弧长微元 $ds$——没有方向，$ds>0$ 恒成立。</p>
<blockquote><b>第二类：沿路做功。</b>力 $\\vec F=(P,Q)$ 在位移 $d\\vec r=(dx,dy)$ 上做功 $Pdx+Qdy$——<b>依赖路径方向</b>，反向积分变号。</blockquote>
<p><b>计算通法：</b>把曲线参数化，"everything 用参数表达，$t$ 从起积到终"。</p>`],
['def', '定义与公式',
`<p><b>① 第一类：</b>$\\displaystyle\\int_L f(x,y)\\,ds$；$L: x=x(t),y=y(t)\\ (\\alpha\\le t\\le\\beta)$：</p>
$$ \\int_\\alpha^\\beta f[x(t),y(t)]\\sqrt{x\'^2(t)+y\'^2(t)}\\,dt $$
<p class="muted">$\\alpha<\\beta$ 恒定（下限必须小）。</p>
...
['warn', '易错点',
`<p>• 第一类积分<b>下限必须小于上限</b>（$ds>0$）；第二类上下限跟方向走。</p>
...
['def', '定义与公式',
`<p><b>① 格林公式：</b>$L$ 为 $D$ 的正向边界（可分段光滑），则</p>
$$ \\oint_L Pdx+Qdy=\\iint_D\\left(\\frac{\\partial Q}{\\partial x}-\\frac{\\partial P}{\\partial y}\\right)dxdy $$
<p><b>② 补线技巧：</b>非闭曲线积分 =（补线成闭路用格林）−（补线段的积分）。</p>
<p><b>③ 积分与路径无关的条件：</b>单连通域内 $\\dfrac{\\partial Q}{\\partial x}=\\dfrac{\\partial P}{\\partial y}$ 恒成立 ⇒ 沿任意路径积分相同，可选最简单路径。</p>`],
['ex', '例题精讲',
...
{id:'q2', q:'路径无关（单连通域）的充要条件是？', opts:['$P=Q$', '$Q_x=P_y$', '$P_y=Q_x$', '$P\\cdot Q=0$'], ans:2, exp:'$\\dfrac{\\partial P}{\\partial y}=\\dfrac{\\partial Q}{\\partial x}$ 处处成立。'}
]
...
['def', '定义与公式',
`<p><b>① 第一类曲面积分计算：</b>投影到 $xOy$ 面，$dS=\\sqrt{1+z_x^2+z_y^2}\\,dxdy$。</p>
<p><b>② 高斯公式：</b>$\\Sigma$ 为闭曲面外侧：</p>
...
<p><b>③ 斯托克斯公式：</b>空间闭曲线积分 ⇔ 绷在其上的曲面积分（格林的空间版）。</p>
<p><b>④ 两类曲面积分联系：</b>通过单位法向量 $\\vec n=(\\cos\\alpha,\\cos\\beta,\\cos\\gamma)$：第二类 = $(P\\cos\\alpha+Q\\cos\\beta+R\\cos\\gamma)$ 对 $dS$ 的第一类积分。</p>`],
['ex', '例题精讲',
...
<div class="ex-box"><div class="ex-t">例 2（高斯）计算 $\\oiint_\\Sigma x\\,dydz+y\\,dzdx+z\\,dxdy$，$\\Sigma$ 为球面外侧</div>
<p>散度 $=1+1+1=3$：</p>
$$ \\iiint_{球}3\\,dV=3\\cdot\\frac43\\pi R^3=4\\pi R^3 $$
</div>`],
['warn', '易错点',
`<p>• 高斯公式要求<b>闭曲面 + 外侧</b>；内侧要变号；不闭先补面（补的面要减掉）。</p>
<p>• 第一类曲面积分投影计算时 $dS$ 的根式因子 $\\sqrt{1+z_x^2+z_y^2}$ 别丢。</p>
<p>• 曲面垂直投影面（如球面投影重合）时不能直接投影，要分块处理。</p>`]
],
quiz: [
