...
['def', '定义与公式',
`<p><b>① 定义：</b>$F\'(x)=f(x)$，则 $F(x)+C$ 称 $f(x)$ 的不定积分，记 $\\displaystyle\\int f(x)dx=F(x)+C$。</p>
<p><b>② 基本积分表（必背）：</b></p>
...
<p class="muted">幂函数积分是"指数加一作分母"，唯独 $x^{-1}$ 特殊给 $\\ln|x|$——绝对值别丢。</p>`],
['ex', '例题精讲',
`<div class="ex-box"><div class="ex-t">例 1 求 $\\displaystyle\\int\\left(2x-\\dfrac{3}{x}+\\dfrac{1}{\\sqrt x}\\right)dx$</div>
<p>逐项积：$\\displaystyle\\int 2x\\,dx=x^2$；$\\displaystyle\\int-\\frac3x dx=-3\\ln|x|$；$\\displaystyle\\int x^{-1/2}dx=2x^{1/2}$。</p>
$$ =x^2-3\\ln|x|+2\\sqrt x+C $$
</div>
...
<p class="muted">有理式积分第一步永远是"凑成积分表里的形状"，加项减项是常用手段。</p></div>
...
{id:'q3', q:'设 $f(x)$ 的原函数为 $\\ln(1+x^2)$，则 $f\'(x)$ 等于？', opts:['$\\dfrac{2x}{1+x^2}$', '$\\dfrac{2x-2x^3}{(1+x^2)^2}$', '$\\dfrac{2}{1+x^2}$', '$\\ln(1+x^2)$'], ans:1, exp:'$f(x)=[\\ln(1+x^2)]\'=\\tfrac{2x}{1+x^2}$，再求导：$f\'=\\tfrac{2(1+x^2)-2x\\cdot2x}{(1+x^2)^2}=\\tfrac{2-2x^2}{(1+x^2)^2}$。'}
]
...
<p><b>常用凑法肌肉记忆：</b>$x\\,dx=\\tfrac12d(x^2)$；$\\tfrac{dx}{x}=d(\\ln x)$；$e^x dx=de^x$；$\\cos x\\,dx=d\\sin x$；$\\tfrac{dx}{\\sqrt x}=2d\\sqrt x$。</p>`],
['def', '定义与公式',
`<p><b>① 第一类换元（凑微分）：</b>$\\displaystyle\\int f[\\varphi(x)]\\varphi\'(x)dx=\\int f[\\varphi(x)]\\,d\\varphi(x)\\xlongequal{u=\\varphi(x)}\\int f(u)du$。</p>
<p><b>② 常用凑微分公式：</b></p>
$$ x\\,dx=\\tfrac12 d(x^2),\\quad \\frac{dx}{\\sqrt x}=2d(\\sqrt x),\\quad \\frac{dx}{x^2}=-d\\!\\left(\\frac1x\\right),\\quad \\frac{dx}{x}=d(\\ln x) $$
...
['def', '定义与公式',
`<p><b>① 第二类换元：</b>令 $x=\\psi(t)$（单调可导、$\\psi\'\\neq0$），算完回代。三大标配：</p>
...
<p>• 三角换元要写清<b> $t$ 的范围</b>保证单调与开方去绝对值正确。</p>
...
<p><b>③ 两大基础积分：</b></p>
$$ \\int\\frac{dx}{x-a}=\\ln|x-a|+C,\\qquad \\int\\frac{dx}{x^2+a^2}=\\frac{1}{a}\\arctan\\frac{x}{a}+C $$`],
['ex', '例题精讲',
