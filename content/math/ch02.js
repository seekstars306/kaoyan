...
<p><b>② 几何意义：</b>$f\'(x_0)$ 是曲线在点 $(x_0, f(x_0))$ 处切线斜率。切线方程 $y-f(x_0)=f\'(x_0)(x-x_0)$；法线斜率 $=-\\dfrac{1}{f\'(x_0)}$。</p>
...
$$ (\\sin x)\'=\\cos x,\\quad (\\cos x)\'=-\\sin x,\\quad (\\tan x)\'=\\sec^2 x=\\frac{1}{\\cos^2 x} $$
...
<p><b>③ 参数方程求导：</b>$x=x(t),\\ y=y(t)$：</p>
$$ \\frac{dy}{dx}=\\frac{y\'(t)}{x\'(t)},\\qquad \\frac{d^2y}{dx^2}=\\frac{\\left(\\dfrac{y\'(t)}{x\'(t)}\\right)_t\'}{x\'(t)} $$
<p class="muted">二阶导极易错：不是 $\\dfrac{y\'\'(t)}{x\'\'(t)}$！要先算一阶导的"商"再对 $t$ 求导、除以 $x\'(t)$。</p>`],
['ex', '例题精讲',
...
{id:'q3', q:'参数方程 $x=t^2,\\ y=t^3$，则 $\\dfrac{dy}{dx}$ 等于？', opts:['$\\dfrac{3}{2}t$', '$\\dfrac{2}{3}t$', '$\\dfrac{3t^2}{2t}$（忘记约分）', '$\\dfrac{2t}{3t^2}$'], ans:0, exp:'$\\dfrac{y\'(t)}{x\'(t)}=\\dfrac{3t^2}{2t}=\\dfrac32 t$——C 忘记约分，是典型错误形态。'}
]
...
['think', '直观理解',
`<p><b>高阶导 = 导数的导数。</b>位移求导是速度，再求导是加速度；图像上看，二阶导描述"切线斜率的变化速度"（为负=越来越平，为正=越来越陡）——这正是后面凹凸性的种子。</p>
...
$$ (\\cos ax)^{(n)}=a^n\\cos\\left(ax+\\tfrac{n\\pi}{2}\\right),\\qquad (\\ln(1+x))^{(n)}=\\frac{(-1)^{n-1}(n-1)!}{(1+x)^n} $$
$$ \\left(\\frac{1}{x+a}\\right)^{(n)}=\\frac{(-1)^n n!}{(x+a)^{n+1}} $$
<p><b>② 莱布尼茨公式</b>（乘积的 $n$ 阶导，类比二项式定理）：</p>
...
<p class="muted">只展开到 $x^2$ 项——低于二阶的 $v^{(k)}$ 全是 0，这就是选 $x^2$ 当 $v$ 的原因。</p></div>
...
<p>• "$f\'\'(x_0)$ 存在"不能由 $f\'$ 连续推出，分段点要用定义。</p>`]
],
quiz: [
