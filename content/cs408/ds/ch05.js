...
['def', '要点与公式',
`<p><b>① 基本量：</b>n 顶点无向图最多 C(n,2)=n(n-1)/2 条边（完全图）；有向图最多 n(n-1) 条弧。<b>连通图至少 n-1 条边</b>；<b>强连通有向图至少 n 条边</b>（成环）。</p>
<p><b>② 度：</b>无向图 Σdeg = 2e；有向图 Σ入度 = Σ出度 = e。</p>
<p><b>③ 存储：</b>邻接矩阵存带权图时权存矩阵、∞ 表无边；邻接表边结点数 = 有向图 e 个、无向图 <b>2e</b> 个。</p>
...
['think', '直观理解',
`<p><b>Prim = 撒网式：</b>从某顶点出发，每次把"离现有树最近"的顶点拉进来。适合<b>稠密图</b> O(n²)。</p>
<blockquote><b>Kruskal = 拣便宜：</b>把所有边从小到大排队，依次尝试加入，不成环就要。适合<b>稀疏图</b> O(e log e)。</blockquote>
<p><b>共同性质：</b>最小生成树<b>边数固定 n-1</b>；权值和唯一；<b>但树的形态可能不唯一</b>（有等权边时）。</p>`],
['def', '算法对比表',
...
<p><b>Dijkstra 手算：</b>每轮在"未确定"里选 dist 最小的 → 标记确定 → 松弛其邻边。n-1 轮完成。</p>`],
['ex', '例题精讲',
...
<p><b>③ 性质：</b>关键路径可能不止一条；<b>只有同时缩短所有关键路径上的公共关键活动，总工期才缩短</b>。</p>`],
['ex', '例题精讲',
